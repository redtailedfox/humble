...
func main() {
	...
	for i := 0; i < consts.NumServers; i++ {
		addr, err := net.ResolveTCPAddr("tcp", fmt.Sprintf(":%d", consts.Decrypt+i))
		if err != nil {
		...
		svr := genericserver.NewServer(
			...
			g,
			server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: "decrypt"}),
